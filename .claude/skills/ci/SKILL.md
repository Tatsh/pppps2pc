...
If any changed files are under `pppps2pc/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `pppps2pc/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `pppps2pc/commands/main.py`,
...
For Python files, strip the `pppps2pc/` prefix and replace `/` with `.` (like module imports).

- Python file `pppps2pc/media.py` → `media:`.
- Multiple files under `pppps2pc/commands/` → `commands:`.
- Single command file `pppps2pc/commands/admin.py` → `commands.admin:`.
