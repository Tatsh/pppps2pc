...
Fixes mypy type errors and eliminates `Any` usage in the pppps2pc project.
...
1. Fix all mypy errors reported by `uv run mypy pppps2pc`
...
Place new TypedDict classes in `pppps2pc/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`pppps2pc/typing.py` when shared.
...
1. Run `uv run mypy pppps2pc` and capture all errors.
...
4. Run `uv run mypy pppps2pc` again to verify fixes.
